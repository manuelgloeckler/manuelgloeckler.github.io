---
layout: article
title: Measuring the local variation of multivariate functions
tags: Math
...
Conversely if $ \mid f'(x) \mid  \leq L \mid $ and be $x, y \in S$ then by the mean value theorem there exists some $\xi \in (x,y)$ such that
...
This now allows us the local Lipschitz criteria.


<dir class="theorem">
 The function $f$ is locally L-Lipschitz on a open and convex set $S$ in norm $ \mid  \mid \cdot  \mid  \mid _a$ and $ \mid  \mid \cdot \mid  \mid _b$ if and only if 
...
Which proofs the reverse direction.<br>
<br>
Now if $f$ is locally L-Lipschitz then for some $x \in S$ and $y = x + \lambda v \in S$ where $\mid \mid v \mid \mid_b = 1$ we have that
...
$$ \begin{aligned}
 \mid  \mid J_f(x) \mid  \mid  &= \sup_{ \mid  \mid v \mid  \mid _b \eq 1}  \mid  \mid  J_f(x) v \mid  \mid _a \\ 
&= \sup_{ \mid  \mid v \mid  \mid _b \eq 1}  \mid  \mid f_v'(x) \mid  \mid _a\\ 
&= L
\end{aligned}
$$
[/proof]
...
By the chain rule, the Jacobian takes to the following form
...
## A probabilistic interpretation of variation
...
$$ \Sigma_{p(v)}[f_v'(x)] = \mathbb{E}_{p(v)}[ J_f(x)v(J_f(x)v)^T] = \frac{1}{d} J_f(x)J_f(x)^T $$

So this is a nice closed-form solution and again involves the Jacobian matrix. Recall these results are induced by the L2 norm, so they also should be related to the operator norm induced by the L2 norm. Recall that $\mid \mid  J_f(x)\mid \mid $ equals the largest singular value and thus the is related to the eigenvalue of $J_f(x)^T J_f(x)$. Be $v$ the corresponding eigenvector then

$$ J_f(x)^T J_f(x) = \lambda v \iff J_f(x)J_f(x)^T(J_f(x)v) = \lambda (J_f(x)v)$$

an hence both share the same eigenvalues. As $J_f(x)J_f(x)^T$ is up to a constant the covariance matrix of directional derivatives, the eigenvector with the largest eigenvalue hence points in the direction of the largest variation as required by the definition of the operator norm.

Yet in contrast to the operator norm, this suggest to regularize all the spectral values of the Jacobian.

## On the variation under non-standard norms

Both the operator norm as well as the covariance matrix of directional derivatives can be hard to compute non-standard norms. We will investigate here the problem in the case of a more general "euclidean-like" norm, the *elliptical norm*.

<dir class="definition">
 An elliptical (pseudo-)norm $\mid \mid  \cdot \mid \mid _S$ is defined for a positive (semi) definite matrix $S$ is given by

  $$  \mid  \mid x \mid  \mid_S = \sqrt{x^T S x} $$
</dir>

It is straightforward to verify that this is indeed a norm. A well-known special case is the *Mahalanobis distance*. We call it *elliptical* as the vectors of norm one lie on an ellipse spanned by the eigenvectors of $A$.
...
$$ A^T S A v = \lambda v \text{ with }  \mid \mid v \mid \mid_2^2 = 1 $$

which is a standard eigenvalue problem on a slightly different matrix $A^T S A$.

It is straightforward to see the only relaying on $\mid \mid \cdot \mid \mid_S$ can also be stated as a generalized eigenvalue problem on this matrix.
...
As a results we have that <br>
$$ v = \frac{w}{ \mid \mid w \mid \mid_S} = \frac{L^{-1}\epsilon} { \mid \mid \epsilon \mid \mid} = L^{-1} \frac{\epsilon}{ \mid \mid \epsilon \mid \mid_2}$$ <br>
It is well known that $\epsilon/ \mid \mid \epsilon \mid \mid_2$ is uniform on the L2 uni-sphere. Note that as $L^{-1}$ is p.d. it is invertible, thus the density of $v$ is given by a change of variable from an unfiform distribution <br>
$$ p_v(x) = C\cdot  \mid\det J  \mid = C \cdot  \mid\det L^{-1} \mid $$<br>
where $C$ is the constant of the uniform base density of $\epsilon/ \mid \mid\epsilon \mid \mid_2$. This density is independent of $x$ thus uniform, proofing the lemma.
[/proof]

As a result we can draw uniform samples from the unit-ellipse by sampling $w \sim \mathcal{N}(w; 0, S^{-1})$ and normalizing them $v = w/ \mid \mid w \mid \mid_S$. Equivalently to the $L2$ case we are now interested in the distribution of directional derivatives. To do this we first require the covariance matrix of this distribution.

$$ \Sigma_v = \mathbb{E}_w \left[ \left(\frac{w}{ \mid \mid w \mid \mid_S}\right)\left(\frac{w}{ \mid \mid w \mid \mid_S}\right)^T \right] = \mathbb{E}_w \left[ \frac{ww^T}{w^T S w} \right]$$

Using the substitution $$w = L^{-1} \epsilon$$ with $\epsilon = \mathcal{N}(0, I)$ and $L^{-1}L^{-T} = S^{-1}$, we obtain that

$$ \mathbb{E}_w \left[ \frac{ww^T}{w^T S w} \right] = \mathbb{E}_\epsilon \left[ \frac{L^{-1}\epsilon \epsilon^T L^{-T}}{ \epsilon^T L^{-T} S L^{-1} \epsilon} \right] =  L^{-1} \mathbb{E}_\epsilon \Sigma_\epsilon \left[ \frac{\epsilon}{ \mid \mid\epsilon \mid \mid_2} \right] L^{-T} = \frac{1}{d} S^{-1} $$

And thus we arrive at a rather similar statement that

$$ \Sigma_{p(v)}[f_v'(x)] = \mathbb{E}_{p(v)}[ J_f(x)v(J_f(x)v)^T] = \frac{1}{d} J_f(x) S^{-1} J_f(x)^T $$

Which again agrees with the results obtained using the operator norm.
